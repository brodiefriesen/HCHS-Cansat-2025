...
function check_host_tools() {
	if [ ! -d ${host_tools} ]; then
		...
		wget https://github.com/nihui/opencv-mobile/releases/download/v28/opencv-mobile-4.10.0-milkv-duo.zip -P ${host_tools}
		unzip ${host_tools}/opencv-mobile-4.10.0-milkv-duo.zip -d ${host_tools}
		rm -rf ${host_tools}/opencv-mobile-4.10.0-milkv-duo.zip
		mkdir ${host_tools}/wiringx/
		wget https://raw.githubusercontent.com/milkv-duo/duo-wiringx/refs/heads/main/src/wiringx.h -P ${host_tools}/wiringx/
		if [ $? -ne 0 ]; then
